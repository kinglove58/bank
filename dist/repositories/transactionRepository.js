    ...
    async withdraw(accountId, amount, description) {
        return await prisma.$transaction(async (tsx) => {
            ...
            if (updatedAccount.count === 0) {
                throw new Error(`Transaction failed: Insufficient funds at the moment of processing`);
            }
    ...
    async transfer(senderAccountId, receiverAccountId, amount, description) {
        return await prisma.$transaction(async (tsx) => {
            ...
            if (senderUpdate.count === 0) {
                throw new Error("Transfer failed: Insufficient funds in sender account at the moment of processing");
            }
            //deposit

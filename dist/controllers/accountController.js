...
export class AccountController {
    ...
    async createAccount(req, res, next) {
        try {
            ...
            const newAccount = await accountService.createNewAccount(req.user.id, type);
    ...
    async getTransactions(req, res, next) {
        try {
            ...
            const result = await accountService.getAccountTransactions(req.user.id, accountNumber, Number(page ?? 1), Number(limit ?? 10), type);
            res.status(200).json({
                status: "success",
                ...result,
            });
